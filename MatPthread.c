...
int main( int argc, char *argv[] )
{
  ...
  double flops = (2 * size * size * size) - size * size; 
double mflops = flops / (exectime / 1000.0) / 1000000.0; 
    double speedup = mflops / (size*size*size / elapsed_time);
  double efficiency = speedup / (double)num_threads;
double scalability = speedup / ((double)size*size*size / elapsed_time / num_threads);
    
  printf("Number of threads: %d\tExecution time:%.3lf sec\n", 
          num_threads, elapsed_time);

  printf("Speedup: %f\tEfficiency: %f\tScalability: %f\n", 
